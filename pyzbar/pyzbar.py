...
__all__ = ["decode", "Point", "Rect", "Decoded", "ZBarSymbol", "EXTERNAL_DEPENDENCIES"]
# zbar_set_verbosity(30) #Uncomment for more debug info

Decoded = namedtuple("Decoded", ["data", "type", "rect", "polygon", "score"])
...
def _decode_symbols(symbols):
    ...
    for symbol in symbols:
        ...
        score = zbar_symbol_get_quality(symbol)
        ...
        yield Decoded(data=data, type=symbol_type, rect=bounding_box(polygon), polygon=polygon, score=score)
...
def _pixel_data(image):
    ...
    if 8 != bpp:
        raise PyZbarError("Unsupported bits-per-pixel [{0}]. Only [8] is supported.".format(bpp))
